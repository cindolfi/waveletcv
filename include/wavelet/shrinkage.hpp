...
struct median
{
    ...
    {
        compute_median(input.getMat(), result);
    }

    void operator()(cv::InputArray input, cv::InputArray mask, cv::Scalar& result) const
    {
        cv::Mat masked_input;
        collect_masked2<T, N>()(input, masked_input, mask);

        compute_median(masked_input, result);
    }

private:
    void compute_median(const cv::Mat& input, cv::Scalar& result) const
    {
        assert(input.channels() == N);
        if (input.total() == 1) {
            result = input.at<Pixel>(0, 0);
        } else {
            cv::Mat channels[N];
